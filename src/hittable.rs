...
impl Hittable for Sphere {
    ...
    fn hit(&self, ray: &Ray, t_min: f64, t_max: f64) -> Option<HitRecord> {
        ...
        fn build_hit_record(ray: &Ray, t: f64, outward_normal: Vec3) -> HitRecord {
            ...
            HitRecord {
                hit_point:  ray.at(t),
                normal: if hitting_front_face { outward_normal } else { outward_normal * -1.0 },
        ...
        if discriminant > 0.0 {
            ...
            if t1 > t_min && t1 < t_max {
                Some (build_hit_record(ray, t1, (ray.at(t1) - self.center).unit_vector()))
            }
            else if t2 > t_min && t2 < t_max {
                Some (build_hit_record(ray, t2, (ray.at(t2) - self.center).unit_vector()))
            }
            else {
                None
            }
